...
<body>
    ...
    <div class="container">
        ...
        $query = "SELECT post.id as post_id, post.*, category.*, user.* FROM `post`, `category`, `user` WHERE category.id=" . $category_id . " AND post.category_id = category.id  AND post.user_id = user.id ORDER BY post.created_at DESC";
